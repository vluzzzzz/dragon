...
async function toggleEditor(itemEl, c) {
  ...
  items.forEach((it, idx) => {
    ...
    box.innerHTML = '<div class="item-title" style="margin-bottom:10px">Destacado ' + (idx + 1) + '</div>'
      ...
      + '<div class="field"><label>Producto vinculado</label><select class="f-fk"><option value="">— ninguno —</option>' + opts + '</select></div></div>'
      + '<div class="mut" style="font-size:11px;margin:-4px 0 10px">Si vinculás un producto, el <b>nombre y precio</b> de esta card se toman del catálogo automáticamente.</div>'
...
function productCard(p) {
  ...
  box.innerHTML =
    ...
    + '<div class="field"><label>Stock disponible</label><label class="switch"><input type="checkbox" class="c-vis"' + (p.is_visible !== false ? ' checked' : '') + '><span class="switch-track"></span></label><div class="mut" style="font-size:11px;margin-top:5px">Si lo apagás, el producto igual se muestra pero en gris con cartel “SIN STOCK”.</div></div></div>'
